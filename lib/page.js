...
module.exports = function(info,colors){
	var invert = colors || false;
...
// 	page += 'a {display: inline-block; margin: 0 40px; text-decoration: none; color: #ffffff; text-transform: uppercase; font-size: 12px;}';
	...
	// set background and text darker
	if(invert){
		// this overwrite has to be done after purecss.io is loaded
		// purple #9900ff
		// blue #3366cc
		// grey darkgrey
		page += '<style>';		
		page += 'body {background-color: black;} body {color: lightgrey;}';
		page += '.pure-table-striped tr:nth-child(2n-1) td {background-color: darkgrey;}';
		page += '</style>';
	}
